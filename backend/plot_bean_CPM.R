...
for (i in 1:ncol(tumor)){
	beanplot(tumor[,i],control[,i],col="lightblue",names=c("Tumor","Control brain"),beanlinewd=0, what=c(0,1,1,1),ylab="log2 CPM")
...
clust<-hclust(dist(as.matrix(result), method="binary"))
plot(clust, main=outFile,  axes=FALSE, sub="", xlab="", ylab="")
dev.off()
